...
void updateDisplay() {
    ...
    if (messageDuration > 0 && currentTime - messageStartTime >= messageDuration) {
        Serial.println("DEBUG: Message duration expired, clearing and turning off display");
        clearDisplay();
        turnOffDisplay();
    }
}
...
static void renderMessage(ScreenMessageType type, const String& text) {
    if (!u8g2) {
        Serial.println("DEBUG: u8g2 is null in renderMessage");
        return;
    }

    Serial.println("DEBUG: Clearing buffer and rendering message");
    u8g2->clearBuffer();
    u8g2->setFont(u8g2_font_ncenB08_tr);
    ...
    Serial.println("DEBUG: Sending buffer to display");
    u8g2->sendBuffer();
    Serial.println("DEBUG: Buffer sent successfully");
}
...
void showMessage(ScreenMessageType type, const String& text, uint32_t duration) {
    ...
    Serial.println("DEBUG: Message rendered successfully");
}
 ...
 */
void turnOffDisplay() {
    Serial.println("DEBUG: Turning OFF display with activity indicators");
    if (u8g2) {
        u8g2->clearBuffer();
 ...
 */
void turnOffDisplayCompletely() {
    Serial.println("DEBUG: Turning OFF display completely for deep sleep");
    if (u8g2) {
        u8g2->clearBuffer();
        u8g2->sendBuffer();
