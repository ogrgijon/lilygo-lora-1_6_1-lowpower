 ...
 *
 * @brief     Archivo de configuración de placas LoRa para LilyGo.
 *            Este archivo contiene todas las funciones necesarias para inicializar
 *            y gestionar los periféricos de las placas LoRa, incluyendo PMU,
 *            display OLED, tarjeta SD, GPS, BLE y otros componentes.
 *
 *            Se ha mejorado con comentarios detallados en español para facilitar
 *            la comprensión y mantenimiento del código.
 */
...
#ifdef HAS_PMU
#endif // Already defined above, remove duplicate
...
/**
 * @brief Inicializa el módulo de gestión de energía (PMU).
 *        Intenta inicializar AXP2101 primero, luego AXP192 si falla.
 *        Configura voltajes, interrupciones y LEDs según el modelo detectado.
 *
...
/**
 * @brief Desactiva los periféricos para ahorrar energía antes de entrar en sueño profundo.
 *        Apaga mediciones de voltaje, interrupciones y canales de energía no utilizados.
 */
...
/**
 * @brief Maneja el bucle de eventos del PMU.
 *        Verifica interrupciones y procesa eventos como inserción/remoción de batería,
 *        carga, y pulsaciones de tecla.
 *
 * @param pressed_cb Callback opcional para pulsaciones cortas de tecla.
 */
 ...
 *        Verifica la conexión I2C y configura el display U8g2.
 *        Opcionalmente muestra un logo de inicialización.
 *
 * @return true si la inicialización es exitosa, false en caso contrario.
 */
bool beginDisplay()
{
    ...
    if (Wire.endTransmission() == 0) {
        ...
        // Descomenta las siguientes líneas para mostrar el logo LilyGo
        /*
        ...
        delay(3000);  // Muestra el logo por 3 segundos
        */

        // ===== OPCIÓN: SIN LOGO =====
        // Pantalla en blanco durante inicialización (ahorro de energía)
        u8g2->clearBuffer();
        u8g2->sendBuffer();

        // ===== OPCIÓN: SIN LOGO =====
        // Comenta las líneas anteriores y descomenta:
        // u8g2->clearBuffer();
        // u8g2->sendBuffer();
...
#ifdef HAS_SDCARD
/**
 * @brief Escribe datos en un archivo de la tarjeta SD.
 *
 * @param path Ruta del archivo en la SD.
 * @param buffer Datos a escribir.
 * @return true si la escritura es exitosa, false en caso contrario.
 */
...
/**
 * @brief Lee datos de un archivo de la tarjeta SD.
 *
 * @param path Ruta del archivo en la SD.
...
/**
 * @brief Prueba la escritura y lectura en la tarjeta SD.
 *        Escribe un mensaje de prueba y lo verifica leyendo de vuelta.
 *
 ...
 *        Configura el SPI y monta la tarjeta SD, luego ejecuta una prueba de escritura/lectura.
 *
 * @return true si la inicialización y prueba pasan, false en caso contrario.
 */
bool beginSDCard()
{
#if defined(HAS_SDCARD) && defined(SD_SHARE_SPI_BUS)
...
/**
 * @brief Inicializa el WiFi en modo Access Point.
 *        Crea un AP con el nombre de la variante de la placa.
 */
void beginWiFi()
{
...
/**
 * @brief Imprime la razón del despertar del sueño profundo.
 *        Muestra en Serial la causa del último despertar (timer, external, etc.).
 */
...
/**
 * @brief Obtiene y muestra información del chip ESP32.
 *        Incluye tamaño de flash, modelo, frecuencia, PSRAM, etc.
 */
void getChipInfo()
{
#if defined(ARDUINO_ARCH_ESP32)
...
/**
 * @brief Configura todas las placas LoRa.
 *        Inicializa Serial, SPI, pines, PMU, SD, display, GPS, etc.
 *        Esta es la función principal de inicialización del hardware.
 *
 * @param disable_u8g2 Si true, no inicializa el display OLED.
 */
void setupBoards(bool disable_u8g2 )
{
    ...
    * */
    ...
    // Indicador visual de inicio con LED (breve parpadeo para ahorro de energía)
#ifdef BOARD_LED
    digitalWrite(BOARD_LED, LED_ON);
    delay(100);
    digitalWrite(BOARD_LED, !LED_ON);
...
/**
 * @brief Imprime el resultado de la inicialización de los dispositivos.
 *        Muestra en Serial y en el display OLED el estado de cada periférico.
 *
 * @param radio_online Estado del módulo de radio LoRa.
 */
...
#ifdef BOARD_LED
/**
 * @brief Hace parpadear el LED de la placa con debounce.
 *        Cambia el estado del LED cada debounceDelay milisegundos.
 */
void flashLed()
{
...
/**
 * @brief Escanea dispositivos I2C en el bus especificado.
 *        Intenta comunicarse con direcciones I2C del 0x01 al 0x7F
 *        e identifica dispositivos conocidos como sensores o displays.
 *
 * @param w Puntero al objeto TwoWire (bus I2C) a escanear.
 */
...
/**
 * @brief Prueba la conexión con el módulo GPS L76K.
 *        Envía comandos para detener NMEA y configurar el módulo.
 *
 * @return true si el módulo responde correctamente, false en caso contrario.
 */
bool l76kProbe()
{
    bool result = false;
 ...
 *        Intenta inicializar L76K hasta 3 veces.
 *
 * @return true si la inicialización es exitosa, false en caso contrario.
 */
bool beginGPS()
...
/**
 * @brief Obtiene el acknowledgment (ACK) del módulo GPS UBX.
 *        Espera una respuesta específica del módulo GPS.
 *
 * @param buffer Buffer para almacenar la respuesta.
 * @param size Tamaño máximo del buffer.
 * @param requestedClass Clase UBX esperada.
 * @param requestedID ID UBX esperado.
 * @return Número de bytes leídos, 0 si falla.
 */
...
/**
 * @brief Recupera la configuración del módulo GPS UBX.
 *        Envía comandos para restaurar configuración por defecto y configurar tasa de navegación.
 *
...
/**
 * @brief Obtiene la temperatura del sensor NTC.
...
/**
 * @brief Configura el módulo BLE (Bluetooth Low Energy).
 *        Inicializa el servidor BLE con un servicio y característica.
 */
void setupBLE()
{
...
/**
 * @brief Calibra el reloj RTC.
 *        Mide la frecuencia del reloj especificado.
 *
 * @param cal_clk Tipo de reloj a calibrar.
 * @param name Nombre del reloj para depuración.
 * @return Valor de calibración.
 */
...
/**
 * @brief Habilita el reloj lento de 32.768 kHz.
 *        Configura el RTC para usar el oscilador de cristal de baja frecuencia.
 */
static void enable_slow_clock()
{
 ...
 *        Lista las redes encontradas con SSID, RSSI, canal y tipo de encriptación.
 */
void scanWiFi()
{
    WiFi.mode(WIFI_STA);
...
/**
 * @brief Lee el voltaje de batería de la forma más fiable disponible.
 *        Si hay PMU, usa el chip AXP192/AXP2101. Si no, usa el ADC y divisor resistivo.
 *        El voltaje máximo esperado es 4.2V (batería 18650 Li-Ion).
 *
 * @return Voltaje de batería en voltios (float).
 */
float readBatteryVoltage() {
    ...
    float vref = 3.3f; // Voltaje de referencia ESP32
    float adcMax = 4095.0f;
    float v_adc = (raw / adcMax) * vref;
    float r1 = BAT_ADC_PULLUP_RES;
    float r2 = BAT_ADC_PULLDOWN_RES;
...
/**
 * @brief Obtiene el porcentaje de batería estimado a partir del voltaje.
 *        3.0V = 0%, 4.2V = 100% (batería 18650 Li-Ion).
 *
 * @param voltage Voltaje de batería en voltios.
